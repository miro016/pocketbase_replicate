...
func serializeModelForReplication(app core.App, model core.Model, op string) (map[string]any, error) {
	...
	if col, ok := model.(*core.Collection); ok {
		return marshalCollectionForCluster(col)
	}
...
func applyCollectionReplication(app core.App, event *cluster.ReplicationEvent) {
	...
	// Fix auth token secrets that may have been blanked or regenerated by
	// collection save hooks (e.g. onCollectionSaveExecute regenerates
	// AuthToken.Secret when AuthRule changes). We overwrite the DB options
	// column with the correct secrets from the source node.
	if col.IsAuth() {
		fixAuthCollectionSecrets(app, col.Id, event.RawData)
	}
}
...
// -----------------------------------------------------------------
// Collection replication helpers — auth secret preservation
// -----------------------------------------------------------------

// marshalCollectionForCluster serializes a collection for cluster replication,
// including auth token secrets that json.Marshal intentionally redacts for API responses.
//
// PocketBase's Collection.MarshalJSON blanks out AuthToken.Secret, FileToken.Secret,
// PasswordResetToken.Secret, EmailChangeToken.Secret, VerificationToken.Secret, and
// OAuth2 provider ClientSecret for security. In the cluster context these secrets
// MUST be preserved so that peer nodes can validate JWT tokens issued by any node.
func marshalCollectionForCluster(col *core.Collection) (map[string]any, error) {
	...
	// For auth collections, inject back the token secrets that MarshalJSON blanked out.
	if col.IsAuth() {
		injectTokenSecret(result, "authToken", col.AuthToken.Secret)
		injectTokenSecret(result, "fileToken", col.FileToken.Secret)
		injectTokenSecret(result, "passwordResetToken", col.PasswordResetToken.Secret)
		injectTokenSecret(result, "emailChangeToken", col.EmailChangeToken.Secret)
		injectTokenSecret(result, "verificationToken", col.VerificationToken.Secret)

		// OAuth2 provider client secrets
		if oauth2Map, ok := result["oauth2"].(map[string]any); ok {
			if providers, ok := oauth2Map["providers"].([]any); ok {
				for i, p := range providers {
					if pMap, ok := p.(map[string]any); ok && i < len(col.OAuth2.Providers) {
						pMap["clientSecret"] = col.OAuth2.Providers[i].ClientSecret
					}
				}
			}
		}
...
// injectTokenSecret sets the "secret" key inside a nested token config map.
func injectTokenSecret(raw map[string]any, key, secret string) {
	if m, ok := raw[key].(map[string]any); ok {
		m["secret"] = secret
	}
}

// fixAuthCollectionSecrets corrects token secrets in the DB that may have been
// blanked or regenerated by collection save hooks during replication.
//
// The onCollectionSaveExecute hook regenerates AuthToken.Secret whenever AuthRule
// changes, producing a random secret that differs from the source node's.
// This function overwrites the DB options column with the correct secrets from
// the incoming replication data, then reloads the collection cache.
func fixAuthCollectionSecrets(app core.App, colId string, rawData map[string]any) {
	// Collect desired secrets from the incoming replication data.
	type secretEntry struct {
		key    string
		secret string
	}
	var desired []secretEntry
	for _, key := range []string{"authToken", "fileToken", "passwordResetToken", "emailChangeToken", "verificationToken"} {
		if tkMap, ok := rawData[key].(map[string]any); ok {
			if s, ok := tkMap["secret"].(string); ok && s != "" {
				desired = append(desired, secretEntry{key, s})
			}
		}
	}
	if len(desired) == 0 {
		return // no secrets in the incoming data (e.g. non-auth or legacy event)
	}

	// Read the current options JSON from the _collections table.
	var optionsJSON string
	if err := app.NonconcurrentDB().NewQuery(
		"SELECT COALESCE(options, '{}') FROM {{_collections}} WHERE id={:id}",
	).Bind(dbx.Params{"id": colId}).Row(&optionsJSON); err != nil {
		app.Logger().Warn("[cluster] fixAuthCollectionSecrets: failed to read options",
			"id", colId, "error", err)
		return
	}
	...
	if err := json.Unmarshal([]byte(optionsJSON), &options); err != nil {
		app.Logger().Warn("[cluster] fixAuthCollectionSecrets: failed to parse options",
			"id", colId, "error", err)
		return
	}

	// Override the token secrets with the values from the source node.
	changed := false
	for _, d := range desired {
		if tkMap, ok := options[d.key].(map[string]any); ok {
			if existing, _ := tkMap["secret"].(string); existing != d.secret {
				tkMap["secret"] = d.secret
				changed = true
			}
		}
	}

	// Also fix OAuth2 provider client secrets.
	if oauth2Raw, ok := rawData["oauth2"].(map[string]any); ok {
		if incomingProviders, ok := oauth2Raw["providers"].([]any); ok {
			if optOAuth2, ok := options["oauth2"].(map[string]any); ok {
				if optProviders, ok := optOAuth2["providers"].([]any); ok {
					for i, p := range incomingProviders {
						if pMap, ok := p.(map[string]any); ok {
							if cs, ok := pMap["clientSecret"].(string); ok && cs != "" && i < len(optProviders) {
								if optP, ok := optProviders[i].(map[string]any); ok {
									if existing, _ := optP["clientSecret"].(string); existing != cs {
										optP["clientSecret"] = cs
										changed = true
									}
								}
							}
	...
	newJSON, err := json.Marshal(options)
	if err != nil {
		app.Logger().Warn("[cluster] fixAuthCollectionSecrets: failed to marshal corrected options",
			"id", colId, "error", err)
		return
	}

	if _, err := app.NonconcurrentDB().NewQuery(
		"UPDATE {{_collections}} SET options={:options} WHERE id={:id}",
	).Bind(dbx.Params{
		"id":      colId,
		"options": string(newJSON),
	}).Execute(); err != nil {
		app.Logger().Warn("[cluster] fixAuthCollectionSecrets: failed to update options",
			"id", colId, "error", err)
		return
	}

	// Reload the collection cache so in-memory state reflects the corrected secrets.
	if err := app.ReloadCachedCollections(); err != nil {
		app.Logger().Warn("[cluster] fixAuthCollectionSecrets: failed to reload cache", "error", err)
	}
}
...
// syncCollectionSchemas sends all collection schemas to a peer, including auth
// token secrets so that peer nodes can validate tokens issued by this node.
// Must be called before syncTable for record tables.
func syncCollectionSchemas(app core.App, send func(*cluster.ReplicationEvent)) {
	...
	for _, col := range collections {
		rawData, err := marshalCollectionForCluster(col)
		if err != nil {
